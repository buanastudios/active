...
let SITE_DATA = { global: {}, products: [], journal: [] };
let JOURNAL = [
  {
    "id": "systems-responsibility",
    "catEn": "Systems", "catId": "Sistem",
    "date": "May 2025",
    "titleEn": "Responsibility Through Implementation", "titleId": "Tanggung Jawab Melalui Implementasi",
    "descEn": "On the discipline of building systems that survive past the person who designed them.",
    "descId": "Tentang disiplin membangun sistem yang bertahan melampaui orang yang merancangnya.",
    "link": "#"
  },
  {
    "id": "archery-focus",
    "catEn": "Discipline", "catId": "Disiplin",
    "date": "April 2025",
    "titleEn": "The Focus of Archery", "titleId": "Fokus dalam Panahan",
    "descEn": "How physical discipline carries into technical work. What the bow teaches that a screen cannot.",
    "descId": "Bagaimana disiplin fisik terbawa ke pekerjaan teknis. Apa yang diajarkan busur yang tidak bisa diajarkan layar.",
    "link": "#"
  },
  {
    "id": "iso-mindset",
    "catEn": "Documentation", "catId": "Dokumentasi",
    "date": "March 2025",
    "titleEn": "The ISO 9001 Mindset", "titleId": "Pola Pikir ISO 9001",
    "descEn": "Why documentation isn't bureaucracy. How a clear process record changes what an organisation is capable of.",
    "descId": "Mengapa dokumentasi bukan birokrasi. Bagaimana catatan proses yang jelas mengubah kemampuan sebuah organisasi.",
    "link": "#"
  },
  {
    "id": "why-offline-first",
    "catEn": "Software", "catId": "Perangkat Lunak",
    "date": "February 2025",
    "titleEn": "Why Offline-First", "titleId": "Mengapa Offline-First",
    "descEn": "Building software for institutions that can't rely on stable internet. The case for local-first data and why it matters in Indonesia.",
    "descId": "Membangun perangkat lunak untuk institusi yang tidak bisa mengandalkan internet stabil. Argumen untuk data local-first dan mengapa itu penting di Indonesia.",
    "link": "#"
  }
];
let PRODUCTS = [
      ...
      <div class="reveal" style="margin-bottom:5rem;">
        <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:2rem; padding-bottom:3rem; border-bottom:1px solid var(--border);">
          ...
          <div style="flex-shrink:0; display:flex; gap:0.5rem; flex-wrap:wrap;">
            ${p.previews && p.previews.length > 0 ? `<button onclick="openPreviewModal('${p.id}')" class="btn btn-ghost" style="border-color:var(--accent); color:var(--accent);"><span class="en">Preview UI</span><span class="id">Pratinjau UI</span></button>` : ''}
...
window.openPreviewModal = function(productId) {
  const modal = document.getElementById('preview-modal');
  const content = document.getElementById('preview-content');
  if (!modal || !content) return;
  
  const product = PRODUCTS.find(p => p.id === productId);
  if (!product || !product.previews || !product.previews.length) return;
  
  // Render gallery
  let html = `<div class="preview-gallery" style="display:flex; overflow-x:auto; gap:1rem; snap-type: x mandatory; padding-bottom:1rem;">`;
  product.previews.forEach(media => {
    html += `<img src="${media}" style="height:70vh; max-height:800px; width:auto; border-radius:var(--radius); border:1px solid var(--border); scroll-snap-align: center; object-fit:contain; background:#000;" alt="Preview">`;
  });
  html += `</div>`;
  
  html += `<h3 style="margin-top:1.5rem;">${product.nameEn} Preview</h3>`;
...
window.closePreviewModal = function() {
  ...
  if (modal) {
    ...
    // Optional: clear content to stop videos if we add them
    setTimeout(() => {
      const content = document.getElementById('preview-content');
...
function initJournal() {
  ...
  grid.innerHTML = JOURNAL.map(j => `
    <a href="${j.link || '#'}" class="journal-item">
      <div class="journal-meta">
        <span class="journal-cat"><span class="en">${j.catEn}</span><span class="id">${j.catId}</span></span>
        <span class="journal-date">${j.date}</span>
      </div>
      <h3><span class="en">${j.titleEn}</span><span class="id">${j.titleId}</span></h3>
      <p><span class="en">${j.descEn}</span><span class="id">${j.descId}</span></p>
    </a>
  `).join('');
}
...
async function boot() {
  ...
  const localPreview = localStorage.getItem('bs-local-data');
  if (localPreview) {
    console.log('Running in Preview Mode: Loaded data from LocalStorage.');
    try {
      SITE_DATA = JSON.parse(localPreview);
      PRODUCTS = SITE_DATA.products || PRODUCTS;
      JOURNAL = SITE_DATA.journal || JOURNAL;
    } catch(e) { console.error('Failed to parse local preview data', e); }
  } else {
    try {
      const res = await fetch('data.json');
      if (res.ok) {
        SITE_DATA = await res.json();
        PRODUCTS = SITE_DATA.products || PRODUCTS; // Fallback to hardcoded if json missing
        JOURNAL = SITE_DATA.journal || JOURNAL;
      }
    } catch (err) {
      console.warn('Running locally: fetch(data.json) blocked by browser CORS policy. Using fallback data.');
    }
  }

  // Initialize reveal observer FIRST so functions can use it
  initReveal();

  // Hydrate DOM with new data
  initProductRotation();
  initHomeProducts();
  initPreviewModal();
  initJournal();
